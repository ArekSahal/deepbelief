...
class DBNet:
    ...
    n_labels,batch_size=10,
    learning_rate=0.05,
    momentum=0.9,
    initial_momentum=0.5, # first 5 spochs
    weight_decay=0.0001,
    tune_rate=0.05, 
    tune_momentum=0.5, 
    tune_decay=0.007, 
    ...
    def set_training_data(self,X,labels,split=0.8):
        if split>=1.0:
            ...
            shuffl_index = torch.randperm(X.shape[0])
            data = X[shuffl_index,:]
    ...
    def predict(self,X):
        """
        Takes in a  X (n_samples x n_features) and return a probability distribution of 
        all the classes (I think) for each sample
        """
        data = X
        data = data.to(device)
        ...
        for i in range(self.n_labels):
            ...
            out[:,i] = energy
        out = torch.div(out.t() ,torch.sum(out, dim=1)).t() # If there is a better way of doing this, pls tell me
        return out.to(device)

    def energy(self,X, lb):
        data = X
        data = data.to(device)
        for m in range(len(self.stack) - 1):
            data = self.stack[m].v_to_h(data, directed=True, sample=False)
    ...
    def generate(self, label,n=200):
        """
        Takes a label as a row tensor and return a list with framems
        Take the last element in the list if you want just the final image
        """
        n_iters = n
        iters = []
        lb = label.to(device)
        data = torch.rand(self.stack[0].n_visible)
    ...
    def daydream(self,X,label, n=200):
        ...
        data = torch.stack([data]).to(device)
        for i in range(n):
            data = self.stack[-1].gibb_sample(data,n=1,sample=False)
            inter_data = data[:,:-self.n_labels]
            for i in torch.arange(len(self.stack)-2, -1,-1):
                if i > 0:
                    inter_data = self.stack[i].h_to_v(inter_data, sample=True, directed=True)
    ...
    def wakesleep_finetune(self, epochs,cd=1,save=True,filename="models/db_"):
        ...
        #for ep in range(epochs):
            ...
            #for batch in range(len(self.batch_indx)):
                ...
                for m in torch.arange(len(self.stack)-2,-1,-1):
                    if m>0:
